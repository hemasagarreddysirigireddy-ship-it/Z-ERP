...
import { Plus, Search, Download, RefreshCw, Eye, Edit, Send, DollarSign, FileText, TrendingUp, Filter, ChevronDown, FileSpreadsheet, Printer } from 'lucide-react';
...
const InvoicesList: React.FC = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(() => {
    return localStorage.getItem('invoices_searchQuery') || '';
  });
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const saved = localStorage.getItem('invoices_itemsPerPage');
  ...
  useEffect(() => {
    localStorage.setItem('invoices_searchQuery', searchQuery);
  }, [searchQuery]);

  // Save items per page to localStorage
  useEffect(() => {
    localStorage.setItem('invoices_itemsPerPage', itemsPerPage.toString());
  }, [itemsPerPage]);
    ...
    <div className="dashboard-layout">
        ...
        <div className="page-container">
          <div className="page-header" style={{ borderBottom: '3px solid #3b82f6', paddingBottom: '1rem', marginBottom: '2rem' }}>
            ...
            <div className="page-header-right">
              <button className="btn-primary" onClick={() => navigate('/sales/invoices/new')} style={{ background: '#3b82f6', fontSize: '1rem', padding: '0.75rem 1.5rem' }}>
                <Plus size={20} />
                New
              </button>
            </div>
      ...
      <div className="table-controls" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <button onClick={fetchInvoices} style={{ padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '6px', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
            <RefreshCw size={18} />
          </button>
        </div>
